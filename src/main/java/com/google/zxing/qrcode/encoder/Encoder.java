...
public final class Encoder {
	 ...
	 * chooseMode(). On success, store the result in "qrCode".
	 * 
	 * We recommend you to use QRCode.EC_LEVEL_L (the lowest level) for "getECLevel" since our primary use is to show QR
	 * code on desktop screens. We don't need very strong error correction for this purpose.
	 * 
	 * Note that there is no way to encode bytes in MODE_KANJI. We might want to add EncodeWithMode() with which clients
	...
	public static void encode(byte[] content, ErrorCorrectionLevel ecLevel, Map<EncodeHintType, ?> hints, QRCode qrCode)
	...
	/**
	 * @return the code point of the table used in alphanumeric mode or -1 if there is no corresponding code in the
	 *         table.
	 */
	 ...
	 * the input is only double-byte Kanji, then we return {@link Mode#KANJI}.
	 */
	...
	/**
	 * Initialize "qrCode" according to "numInputBits", "ecLevel", and "mode". On success, modify "qrCode".
	 */
	private static void initQRCode(int numInputBits, ErrorCorrectionLevel ecLevel, Mode mode, QRCode qrCode)
			throws WriterException {
	...
	static void interleaveWithECBytes(BitArray bits, int numTotalBytes, int numDataBytes, int numRSBlocks,
		...
		if (numTotalBytes != result.getSizeInBytes()) { // Should be same.
			throw new WriterException("Interleaving error: " + numTotalBytes + " and " + result.getSizeInBytes()
					+ " differ.");
		}
	}
	...
	static void append8BitBytes(byte[] bytes, BitArray bits) throws WriterException {
		for (byte b : bytes) {
			bits.appendBits(b, 8);
		}
	}
